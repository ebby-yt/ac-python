...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            global_active = evaluate_global_activity(raw_metric_values)
            if sequential_error:
                print("[display] Sequential read error detected; using available sensor values for active state.")
            for entry in strip_metadata:
                entry['active'] = global_active
            if args.single_strip:
                # Legacy renderer keeps the original one-strip metric slices for debugging.
                render_single_strip(strip, strip_metadata, cleaned_metric_values, gradients)
            else:
                render_physical_strip_blocks(strip_entries, gradients, global_active)
            m = None
            time.sleep(2)
